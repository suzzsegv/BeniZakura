 ...
 */
bool CColor_Comment_Cpp_If1::BeginColor( const CStringRef& rStr, int pos, ColorStrategyState& rCommentNestLevel )
{
	if( !rStr.IsValid( ) ){
		return false;
	}
	...
	if( Match_CommentFrom( pos, rStr, rCommentNestLevel ) ){
		this->commentEndPos = Match_CommentTo( pos + wcslen( L"#if 0" ), rStr, rCommentNestLevel );
		return true;
	}
...
/*!
	...
	@retval false: 検出しなかった
 */
bool CColor_Comment_Cpp_If1::EndColor( const CStringRef& rStr, int pos, ColorStrategyState& rCommentNestLevel )
{
	if( this->commentEndPos == 0 ){
		this->commentEndPos = Match_CommentTo( pos, rStr, rCommentNestLevel );
