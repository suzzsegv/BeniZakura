...
//	case F_WORDSREFERENCE:			Command_WORDSREFERENCE();break;		/* 単語リファレンス */
	...
	case F_GOLINETOP:		Command_GOLINETOP( m_pCommanderView->GetSelectionInfo().m_bSelectingLock, lparam1 | 1 ); break;	//行頭に移動(折り返し単位/改行単位)
...
//	case F_GOLOGICALLINEEND_BOX:if( ! this->m_pCommanderView->GetSelectionInfo().IsBoxSelecting() ) { this->Command_BEGIN_BOXSELECT( false ); } this->Command_GOLINEEND( true, 0, 8 | lparam1 );break;	//(矩形選択)行末に移動(改行単位)
	...
	case F_SYMBOL_SEARCH:	Command_TagJumpByTagsFileKeyword( (const wchar_t*)lparam1 );break;
...
// To Here 2001.12.03 hor
	...
	case F_CANCEL_MODE:		Command_CANCEL_MODE();			//各種モードの取り消し
							Command_SEARCH_CLEARMARK();		//検索マークのクリア
							break;
