...
int CSakuraEnvironment::_ExParam_Evaluate( const wchar_t* pCond )
{
	...
	case L'X': // ファイルの排他制御
		if( CAppMode::getInstance()->GetFileShareMode() == SHAREMODE_NOT_EXCLUSIVE ){
			return 0; // 排他制御: しない
		}else if( CAppMode::getInstance()->GetFileShareMode() == SHAREMODE_DENY_WRITE ){
 			return 1; // 排他制御: 上書き禁止
		}else{
 			return 2; // 排他制御: 読み書き禁止
 		}
	default:
		break;
	}
